...
impl AttackState {
    ...
    // ATTACK.HSQ's own entries onto the live palette, leaving everything else
    // intact. We mirror that by seeding the working palette from `seed` before
    ...
    pub fn new(seed: &Palette, spritesheet_data: &[u8]) -> Self {
        ...
        // Seed the base palettes with the live palette; load_spritesheet then
        // overlays ATTACK.HSQ's entries on top (= apply_sprite_sheet_palette).
        attack.gfx.pal_2bf.copy_from(seed);
    ...
    fn load_spritesheet(&mut self, spritesheet_data: &[u8]) {
        self.sprite_sheet = SpriteSheet::from_possibly_compressed_slice(spritesheet_data).unwrap();
