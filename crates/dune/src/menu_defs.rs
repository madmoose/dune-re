...
pub(crate) struct MenuItem {
    pub text_id: u16,
    pub handler: u16,
    pub callback: fn(state: &mut GameState),
}
...
pub(crate) const fn item(
    text_id: u16,
    handler: u16,
    callback: fn(state: &mut GameState),
) -> MenuItem {
    MenuItem {
        text_id,
        handler,
...
const fn gray(text_id: u16, handler: u16, callback: fn(state: &mut GameState)) -> MenuItem {
    item(text_id | CMD_GREY, handler, callback)
}

const fn highlight(text_id: u16, handler: u16, callback: fn(state: &mut GameState)) -> MenuItem {
...
pub(crate) const MENU_COMMS_ROOM_MESSAGES_VIEWED: MenuDef = menu(0xfb, &[
    item(cmd::VIEWED, 0x2993, |_| println!("menu: Viewed (seg000:2993) not ported")),
    item(cmd::WHAT,   0x9ed5, GameState::menu_callback_choice_what),
]);
...
pub(crate) const MENU_ARGUE_ACCEPT_REFUSE: MenuDef = menu(0xfb, &[
    item(cmd::ARGUE,  0x2453, |_| println!("menu: ARGUE (seg000:2453) not ported")),
    item(cmd::ACCEPT, 0x241a, |_| println!("menu: ACCEPT (seg000:241a) not ported")),
    item(cmd::REFUSE, 0x2432, |_| println!("menu: REFUSE (seg000:2432) not ported")),
    item(cmd::WHAT,   0x9ed5, GameState::menu_callback_choice_what),
]);
...
pub(crate) const MENU_BOOK: MenuDef = menu(0xff, &[
    item(cmd::ALL_TOPICS,         0xaf58, |state| state.menu_callback_choice_book_topic(0)),
    item(cmd::TOPIC_PAUL_ON_DUNE, 0xaf60, |state| state.menu_callback_choice_book_topic(1)),
    item(cmd::TOPIC_SPICE,        0xaf68, |state| state.menu_callback_choice_book_topic(2)),
    item(cmd::TOPIC_THE_FREMEN,   0xaf70, |state| state.menu_callback_choice_book_topic(3)),
    item(cmd::CLOSE_BOOK,         0xb18b, GameState::callback_ui_element_book_close),
]);
...
pub(crate) const MENU_GLOBE: MenuDef = menu(0xff, &[
    item(cmd::EXIT_GLOBE,  0xbc81, |_| println!("menu: EXIT GLOBE (seg000:bc81) not ported")),
    item(cmd::SEE_RESULTS, 0xb96b, |_| println!("menu: SEE RESULTS (seg000:b96b) not ported")),
...
pub(crate) const MENU_GLOBE_DEFAULT_CLICK_ON_GLOBE: MenuDef = menu(0xff, &[
    highlight(cmd::SEE_MAP_OF_THIS_AREA, 0x0f66, |_| {}),
]);
...
/// = seg001:207a menu_globe_save_game — DOS passes the row in cx; the port
/// derives the slot from the row's fixed label id (0x10f/0x110).
#[rustfmt::skip]
pub(crate) const MENU_SAVE_GAME: MenuDef = menu(0xfe, &[
    item(cmd::LOG_1_DAY_0_12_00_A_M, 0xb35a, |state| state.menu_callback_choice_globe_save_game(0, cmd::LOG_1_DAY_0_12_00_A_M)),
    item(cmd::LOG_2_DAY_0_12_00_A_M, 0xb35a, |state| state.menu_callback_choice_globe_save_game(1, cmd::LOG_2_DAY_0_12_00_A_M)),
...
/// autosave slots; the slot again follows from the fixed label id
/// (0x10f..0x112).
#[rustfmt::skip]
pub(crate) const MENU_LOAD_GAME: MenuDef = menu(0xfe, &[
    item(cmd::LOG_1_DAY_0_12_00_A_M,      0xb3b0, |state| state.menu_callback_choice_globe_load_game(0)),
    item(cmd::LOG_2_DAY_0_12_00_A_M,      0xb3b0, |state| state.menu_callback_choice_globe_load_game(1)),
    item(cmd::LAST_ENTERING_INTO_A_PLACE, 0xb3b0, |state| state.menu_callback_choice_globe_load_game(2)),
    item(cmd::LAST_ENTERING_NEW_SIETCH,   0xb3b0, |state| state.menu_callback_choice_globe_load_game(3)),
    item(cmd::CANCEL,                     0xd2e2, GameState::menu_callback_choice_exit_menu),
]);
...
pub(crate) const MENU_RESTART_LOAD_EXIT_GAME: MenuDef = menu(0xff, &[
    item(cmd::RESTART_GAME, 0x0e47, |_| println!("menu: RESTART GAME (seg000:0e47) not ported")),
    item(cmd::LOAD_GAME,    0xb29e, GameState::menu_callback_choice_mirror_room_load_game),
    item(cmd::EXIT_GAME,    0x0e3e, GameState::menu_callback_choice_exit_game),
    item(cmd::WHAT,         0x9ed5, GameState::menu_callback_choice_what),
]);
...
pub(crate) const MENU_EXIT_GAME_CONFIRMATION: MenuDef = menu(0xf6, &[
    item(cmd::YES_I_WANT_TO_EXIT_GAME, 0x003a, |state| state.exit_to_dos()),
    item(cmd::NO_I_WISH_TO_CONTINUE,   0xd2e2, GameState::menu_callback_choice_exit_menu),
]);

/// = seg001:20c2 menu_palace_mirror_room — the "Look away from the mirror" row
/// routes through dispatch_command_handler: the ported glue there pops the
/// mirror overlay before running look_away_from_mirror (private to
/// room_game_screen).
#[rustfmt::skip]
pub(crate) const MENU_PALACE_MIRROR_ROOM: MenuDef = menu(0xff, &[
    ...
    item(cmd::SAVE_GAME,                 0xb28c, GameState::menu_callback_choice_mirror_room_save_game),
    item(cmd::EXIT_GAME,                 0x0e3e, GameState::menu_callback_choice_exit_game),
    item(cmd::LOOK_AWAY_FROM_THE_MIRROR, 0x0eb9, |state| state.dispatch_command_handler(0x0eb9, cmd::LOOK_AWAY_FROM_THE_MIRROR)),
]);
...
pub(crate) const MENU_MAP_TROOPS: MenuDef = menu(0xff, &[
    item(cmd::EXIT_MAPS,             0x186b, GameState::ui_toggle_room_view),
    ...
    item(cmd::FIND_PROSPECTORS,      0x5b1e, |_| println!("menu: FIND PROSPECTORS (seg000:5b1e) not ported")),
]);
...
pub(crate) const MENU_TROOP_DIALOG: MenuDef = menu(0xfc, &[
    ...
    item(cmd::MODIFY_EQUIPMENT,         0x7cbb, |_| println!("menu: MODIFY EQUIPMENT (seg000:7cbb) not ported")),
...
pub(crate) const MENU_MOVE_PROSPECTORS: MenuDef = menu(0xf8, &[
    item(cmd::ADD_A_DESTINATION,     0x80c7, |_| {}),
...
pub(crate) const MENU_OCCUPATION_FOR_SPICE_TROOP: MenuDef = menu(0xf8, &[
    item(cmd::GO_SEARCH_FOR_EQUIPMENT, 0x776d, |_| println!("menu: GO & SEARCH FOR EQUIPMENT (seg000:776d) not ported")),
    item(cmd::SPECIALIZE_IN_ARMY,      0x6a83, GameState::menu_callback_choice_troop_occupation_specialize_in_army),
...
pub(crate) const MENU_OCCUPATION_FOR_ARMY_TROOP: MenuDef = menu(0xf8, &[
    item(cmd::GO_SEARCH_FOR_EQUIPMENT, 0x7734, |_| println!("menu: GO & SEARCH FOR EQUIPMENT (seg000:7734) not ported")),
    gray(cmd::ESPIONAGE_5F,            0x6a45, |_| println!("menu: ESPIONAGE (seg000:6a45) not ported")),
    item(cmd::SPECIALIZE_IN_SPICE,     0x6a71, GameState::menu_callback_choice_troop_occupation_specialize_in_spice),
    item(cmd::SPECIALIZE_IN_ECOLOGY,   0x6a87, GameState::menu_callback_choice_troop_occupation_specialize_in_ecology),
    item(cmd::CANCEL,                  0xd2e2, GameState::menu_callback_choice_exit_menu),
]);
...
pub(crate) const MENU_OCCUPATION_FOR_ESPIONAGE_TROOP: MenuDef = menu(0xf8, &[
    item(cmd::ATTACK, 0x6a2f, |_| println!("menu: ATTACK (seg000:6a2f) not ported")),
    item(cmd::CANCEL, 0xd2e2, GameState::menu_callback_choice_exit_menu),
]);
...
pub(crate) const MENU_OCCUPATION_FOR_ECOLOGY_TROOP: MenuDef = menu(0xf8, &[
    item(cmd::GO_SEARCH_FOR_EQUIPMENT, 0x775c, |_| println!("menu: GO & SEARCH FOR EQUIPMENT (seg000:775c) not ported")),
