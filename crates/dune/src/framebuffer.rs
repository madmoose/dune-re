...
impl FrameBuffer {
    ...
    /// Write the framebuffer to a PNG at its native 1:1 size (320x200 for the
    /// game screen), resolving each indexed pixel through `pal`.
    pub fn write_png<P: AsRef<Path>>(&self, pal: &Palette, path: P) -> std::io::Result<()> {
        self.write_png_(pal, path.as_ref())
    }

    fn write_png_(&self, pal: &Palette, path: &Path) -> std::io::Result<()> {
        ...
        let mut encoder = png::Encoder::new(buf, self.w as u32, self.h as u32);
